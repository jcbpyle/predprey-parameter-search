...
FILE *count_output;
int pd = 0, changeover = 0, nz = 0, curr = 0, initialGrass = 0, initialPred = 0, initialPrey = 0, iteration = 0;
...
__FLAME_GPU_INIT_FUNC__ void initLogFile(){
	...
	sprintf(output_file, "%s%s", getOutputDir(), "simulation_results.csv");
...
__FLAME_GPU_EXIT_FUNC__ void closeLogFile(){
	...
	fprintf(count_output, "Parameters,%d,%d,%d,%f,%f,%d,%d,%d,", initialPrey, initialPred, initialGrass, *get_REPRODUCE_PREY_PROB(), *get_REPRODUCE_PREDATOR_PROB(), *get_GAIN_FROM_FOOD_PREY(), *get_GAIN_FROM_FOOD_PREDATOR(), *get_GRASS_REGROW_CYCLES());
	fprintf(count_output, "Fitnesses,%d,%f,%d,Iterations,%d\n", changeover, float(pd)/iteration, nz, iteration);
